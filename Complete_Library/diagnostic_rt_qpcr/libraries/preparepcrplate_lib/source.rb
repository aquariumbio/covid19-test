...
module PreparePCRPlate_Lib
  ...
  def run_protocol(operations:)
    setup_input_stripwells(operations: operations, array_key: TEMPLATE) if debug
    ...
    show_result(collection: operations.first.input(PLATE).collection) if debug
    ...
    operations.each do |op|
      ...
      op.input_array(TEMPLATE).each { |fv| fv.collection.mark_as_deleted }
    end
  ...
  def add_all_samples(operation:)
    collection = operation.input(PLATE).collection
  ...
  def add_control_samples(compositions:, microtiter_plate:,
    ...
    remaining_compositions = compositions
    CONTROL_TYPE_STUBS.each do |stub|
      ...
      compositions, remaining_compositions = remaining_compositions.partition do |c|
        c.template.sample.name == name
      end

      next unless compositions.present?
