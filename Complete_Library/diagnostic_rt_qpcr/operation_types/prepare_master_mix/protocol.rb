...
class Protocol
  ...
  # TODO The labeling isn't robust but will work for current testing
  ...
  def show_label_mmix_tubes(labels:)
    n = labels.length
    labels = labels.map { |label| "<b>#{label.chars.last(2).join}</b>" }
    show do
  ...
  def show_pipet_mmix(primer_mix_name:, volume:, collection:, layout_group:)
    show do
      title "Pipet <b>#{primer_mix_name.chars.last(2).join}</b> master mix into plate"

      note "Pipet #{volume} #{MICROLITERS} of <b>#{primer_mix_name.chars.last(2).join}</b>" \
  ...
  def primer_probe_table(compositions:, sample_number:)
    ...
    compositions.each do |composition|
      ...
      row = [
        pp_mix.sample.name.chars.last(2).join,
        pp_mix.item.to_s,
