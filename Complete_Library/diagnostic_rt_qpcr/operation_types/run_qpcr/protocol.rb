...
class Protocol
  ...
  def vortex_plate(ops)
    ops.each do |op|
      next unless op.temporary[:options][:program_name].include?('Modified_CDC')
      collection = op.input(PLATE).collection
      collection.get_non_empty.group_by{ |loc| loc[0] }.each do |row, rc_group|
        show do
          title 'Vortex Samples'
          note "Individually vortex each stripwell in"\
            " <b>#{collection.id}</b>"
          note 'Votex each stripwell for 5 2 second bursts'
          separator
          note "Be sure to return each stripwell"\
          ' in the right orientation'
          table highlight_collection_rc(collection, rc_group){ |r, c| c }
        end
      end
    end
  end
  ...
  def start_thermocyclers(paired_ops)
    ...
    paired_ops.group_by{|op| op.get(THERMOCYCLER_KEY)}.each do |thermo_type, ops|
      ...
      if ops.length == 1 && !ops.first.temporary[:options][:program_name].include?('Modified_CDC')
  ...
  def pair_ops_and_thermocyclers(thermocyclers, operations)
    ...
    operations.group_by{ |op| 
      ...
      thermocyclers.each do |thermo|
        ...
        if !program.include?('Modified_CDC')
          ops.each do |op|
            next if paired_ops.include?(op)
