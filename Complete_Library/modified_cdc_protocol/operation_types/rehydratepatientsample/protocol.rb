...
class Protocol
  ...
  def main
    setup_input_stripwells(operations: operations, array_key: SPECIMEN) if debug
    ...
    operations.each do |op|
      dem = get_desired_dimensions(op)
      operation_params = op.temporary[:options]
      add_qty_display(operation_params)

      array_pass(op, SPECIMEN)
      item_list = op.input_array(SPECIMEN).map{ |fv|
        fv.collection.collection? ? fv.collection : fv.item
      }
      media = op.input(MEDIA).item

      sample_rack = TubeRack.new(dem[0], dem[1], name: "#{TUBE_MICROFUGE} rack")

      show_fetch_tube_rack(sample_rack)

      show_add_items(item_list, sample_rack)

      show_fill_reservoir(media, pour: true)
    ...
    operations.each { |op| op.input('Media').item.mark_as_deleted }
  ...
  def get_desired_dimensions(op)
    if op.input_array(SPECIMEN).first.collection.dimensions[1] > 8
      return [8, 12]
    else
      return [12, 8]
    end
  ...
  def vortex_samples(sample_rack:, rc_list: nil)
    ...
    show do
      ...
      table highlight_tube_rack_rc(sample_rack, rc_list){ |r, c| c }
    end
  end
