...
class Protocol
  ...
  def main
    ...
    provision_pcr_plates(operations: operations)
  ...
  # TODO:  This really should be solved with the plate layout generator
  def provision_pcr_plates(operations:)
    operations.each do |op|
      ...
      num_loops = nil
      if options[:stripwell_id]
        stripwells = options[:stripwell_id]
        num_loops = stripwells.length * Collection.find(stripwells.first).dimensions[1]
      else
        num_loops = options[:num_stripwells] * 
                    Collection.where(object_type: options[:object_type]).first.dimensions[1]
      end
  ...
  def set_group_size(operations:)
    operations.each do |op|
      given_stripwells = op.temporary[:options][:stripwell_id]
      object_type = op.temporary[:options][:object_type]
      columns = nil
      if given_stripwells
        columns = Collection.find(given_stripwells.first).dimensions[1]
      else
        columns = Collection.where(object_type: ObjectType.find_by_name(object_type)).first.dimensions[1]
      end
      op.temporary[:options][:group_size] = columns
    end
  end
  ...
  def error_ops
    ...
    show do
      title 'Job Failed'
      note 'The job failed.  Please talk with the lab manager'
    end
  end
